...
export class FakePullRequestSource implements PullRequestSource {
  constructor(private readonly items: PullRequest[] = sampleData) {}

  OpenPullRequest(repos: string[]): Promise<PullRequest[]> {
    return Promise.resolve(this.items.filter((pullRequest) => repos.includes(pullRequest.repo)));
  }
}

const sampleData: PullRequest[] = [
